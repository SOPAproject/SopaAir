package
{
	...
	public class SopaBeamer
	{
		...
		private var sopaAtt:Vector.<Number> = new Vector.<Number>(256);
		
		private var _isRunning:Boolean;
		private var _isFailed:Boolean;
		private var _isReady:Boolean;
		private var _isFocusOn:Boolean;
		private var _isSharp:Boolean;
		...
		private var nAmp:Vector.<Number>;
		...
		[Embed(source="hrtf3d512.bin", mimeType="application/octet-stream")]
		private static const HRTF_DATA:Class;
		[Embed(source="phase3d512.bin", mimeType="application/octet-stream")]
		private static const PHASE_DATA:Class;

		public function SopaBeamer()
		{
			_isRunning = false;
			_isFailed = false;
			_isReady = false;
		...
		public function set isFocusOn(isIt:Boolean):void{
			_isFocusOn = isIt;
		}
		
		public function set isSharp(isIt:Boolean):void{
			_isSharp = isIt;
		}
		...
		public function set isReady(bReady:Boolean):void{
			_isReady = bReady;
		}
		
		public function set isFailed(bIsit:Boolean):void{
			_isFailed = bIsit;
		}
		
		public function get isFocusOn():Boolean{
			return _isFocusOn;
		}
		
		public function get isSharp():Boolean{
			return _isSharp;
		}
		...
		public function sopaPrepare():void{
			...
			if(hrtfVect.length == 0){
				...
				hrtfLoad.addEventListener(IOErrorEvent.IO_ERROR,hrtfIOerror);
			...
			if(phaseVect.length == 0){
				...
				phaseLoad.addEventListener(IOErrorEvent.IO_ERROR,phaseIOerror);
				phaseLoad.load(new URLRequest(binStr));
			}
			
			for(var iSectNum:int = 0;iSectNum < 254;iSectNum ++){
				coordVect[iSectNum] = initCoord(iSectNum);
				nAngl = Math.atan2(Math.sqrt(1 - coordVect[iSectNum].z * coordVect[iSectNum].z),-coordVect[iSectNum].z);
				sopaAtt[iSectNum] = (1 + Math.cos(nAngl)) / 2;
//				sopaAtt[iSectNum] *= sopaAtt[iSectNum];
...
/*						if(iTilt >= 18)
							nTilt = -Math.PI * (36 - Number(iTilt)) / 36;
						else	*/
		...
		private function phaseIOerror(event:IOErrorEvent):void{
			var iVal:int;
			
			var phaseDataObj:Object = new PHASE_DATA();
			var phaseStream:ByteArray = phaseDataObj as ByteArray;
			phaseStream.endian = Endian.LITTLE_ENDIAN;
			while(phaseStream.bytesAvailable){
				iVal = phaseStream.readShort();
				phaseVect.push(iVal);
			}	
			if(phaseVect.length != 130048)
				_isFailed = true;
		...
		private function hrtfIOerror(event:IOErrorEvent):void{
			var iVal:int;
			
			var hrtfDataObj:Object = new HRTF_DATA();
			var hrtfStream:ByteArray = hrtfDataObj as ByteArray;
			hrtfStream.endian = Endian.LITTLE_ENDIAN;
			while(hrtfStream.bytesAvailable){
				iVal = hrtfStream.readShort();
				hrtfVect.push(iVal);
			}	
			if(hrtfVect.length != 130048)
				_isFailed = true;
		...
		private function soundDataHandler(event:SampleDataEvent):void{
			...
			var angleSecVect:Vector.<int> = new Vector.<int>(iHalf);
			...
			for(iCnt = 0;iCnt < iFrameNum;iCnt ++){
				...
				if(iOverlap == 2){
					if(iSopaVersion >= 3){
						angleSecVect = anglStream.splice(0,iHalf);
						while(angleSecVect.length < iHalf)
							angleSecVect.push(0);
					}
					else
				...
				if(!fastFt(false)){
					...
					for(iBin = 0;iBin < iHalf;iBin ++){
						...
						if(iSopaVersion < 2){
							...
							iAngl = opposite(iAngr);
							if(iSopaVersion >= 3){
								iAngrSec = angleSecVect[iBin];
								iAnglSec = opposite(iAngrSec);
							}
						}
						if(iAngl > 255 || iAnglSec > 255){
							_iSamplesPerChannel = _nSamplesDone;
							break;						
						}
						...
						if(_horizontalAngle == 0)
							iAngl = dirArray[iAngl][_horizontalAngle][_verticalAngle];
						else
							iAngl = dirArray[iAngl][72 - _horizontalAngle][_verticalAngle];
						if(iSopaVersion >= 3){
							iAngrSec = dirArray[iAngrSec][_horizontalAngle][_verticalAngle];
							if(_horizontalAngle == 0)
								iAnglSec = dirArray[iAnglSec][_horizontalAngle][_verticalAngle];
							else
								iAnglSec = dirArray[iAnglSec][72 - _horizontalAngle][_verticalAngle];
						}
						...
						if(iBin == 0 || iAngr >= 254){
							...
							if(iSopaVersion >= 3){
								iSecond = 512 * iAngrSec + iFreq;
								nPwr += Number(hrtfVect[iSecond]) / nAtt;
								nPwr /= 2;
							}
							if(_isFocusOn){
								if(!_isSharp){
									if(iSopaVersion >= 3){
										nAmp[iBin] += (sopaAtt[iAngl] + sopaAtt[iAnglSec]) / 2.0;
									}
									else
										nAmp[iBin] += sopaAtt[iAngl];
									nAmp[iBin] /= 2.0;
//									nPwr *= sopaAtt[iAngl];
									nPwr *= nAmp[iBin];
								}
									...
									if(iSopaVersion >= 3)
										nAmp[iBin] += (sopaAtt[iAngl] * sopaAtt[iAngl] + sopaAtt[iAnglSec] * sopaAtt[iAnglSec]) / 2.0;
									else
										nAmp[iBin] += sopaAtt[iAngl] * sopaAtt[iAngl];
									nAmp[iBin] /= 2;									
//									nPwr *= sopaAtt[iAngl] * sopaAtt[iAngl];
									nPwr *= nAmp[iBin];
								}
							}
							var nPhase:Number = Number(phaseVect[iNumber]) / 10000.0;
							if(iSopaVersion >= 3){
								nPhase += Number(phaseVect[iSecond]) / 10000.0;
								nPhase /= 2.0;
								if(Math.abs(phaseVect[iNumber] - phaseVect[iSecond]) > 31415){
									if(nPhase < 0)
										nPhase += Math.PI;
									else
										nPhase -= Math.PI;
							...
							if(_isFocusOn){
								if(!_isSharp){
									if(iSopaVersion >= 3){
										nAmp[iMirror] += (sopaAtt[iAngl] + sopaAtt[iAnglSec]) / 2.0;
									}
									else
										nAmp[iMirror] += sopaAtt[iAngl];
									nAmp[iMirror] /= 2;
//									nPwr *= sopaAtt[iAngl];
									nPwr *= nAmp[iMirror];
								}
									...
									if(iSopaVersion >= 3)
										nAmp[iMirror] += (sopaAtt[iAngl] * sopaAtt[iAngl] + sopaAtt[iAnglSec] * sopaAtt[iAnglSec]) / 2.0;
									else
										nAmp[iMirror] += sopaAtt[iAngl] * sopaAtt[iAngl];
									nAmp[iMirror] /= 2;									
//									nPwr *= sopaAtt[iAngl] * sopaAtt[iAngl];
									nPwr *= nAmp[iMirror];
								}
